...
export default function LoginPage() {
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4 py-8 text-white sm:px-6">
      <AuthBackground videoSrc="/login-bg.mp4" reverse={false} />
        ...
        <div className="grid items-center gap-8 lg:grid-cols-2 lg:gap-16">
          ...
          >
            ...
            <h1 className="mt-8 text-5xl font-black leading-tight tracking-tight">
              Создавай <span className="gradient-text">продающие</span>
              <br />
              карточки для всех
              <br />
              маркетплейсов
            ...
            <p className="mt-6 max-w-xl text-lg leading-8 text-white/[0.62]">
              Один сервис для Uzum, Wildberries, Ozon и Яндекс Маркета. AI генерирует
              премиум-дизайн за 30 секунд.
            </p>
            ...
            <div className="mt-10 flex items-center gap-8">
              <div>
                <div className="gradient-text text-3xl font-black">50,000+</div>
                <div className="text-sm text-white/[0.48]">карточек создано</div>
              </div>
              <div className="h-12 w-px bg-white/12" />
              <div>
                <div className="gradient-text text-3xl font-black">3,500+</div>
                <div className="text-sm text-white/[0.48]">активных продавцов</div>
              </div>
            </div>
          </motion.aside>
          ...
          <motion.section
            ...
            transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
          >
            <div className="premium-reflection relative overflow-hidden rounded-[2rem] border border-white/12 bg-white/[0.065] p-6 shadow-[0_0_90px_rgba(56,189,248,0.14)] backdrop-blur-2xl sm:p-8 lg:p-10">
              <motion.div
                animate={{ rotate: 360 }}
                transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
                className="absolute -right-10 -top-10 h-32 w-32 rounded-full border border-cyan-300/12"
              />
              <motion.div
                animate={{ rotate: -360 }}
                transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
                className="absolute -bottom-8 -left-8 h-24 w-24 rounded-full border border-fuchsia-300/12"
              />
              ...
              <div className="mb-8">
                <p className="mb-3 inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/[0.08] px-3 py-1.5 text-xs font-bold uppercase tracking-[0.2em] text-cyan-100">
                  <Sparkles className="h-3.5 w-3.5" />
                  MarketCard Studio
                </p>
                <h2 className="text-3xl font-black tracking-tight lg:text-4xl">
                  {forgotMode ? "Сброс пароля" : "Вход в аккаунт"}
                </h2>
                <p className="mt-2 text-white/[0.54]">
                  {forgotMode
                    ? "Введите email, и мы отправим письмо для восстановления доступа."
                    : "Войди, чтобы продолжить создавать карточки."}
                </p>
              </div>
              ...
              {forgotMode ? (
                <button
                  ...
                  className="mt-6 w-full text-center text-sm font-semibold text-white/[0.58] transition hover:text-white"
                >
                  Вернуться ко входу
                </button>
              ) : (
...
function AuthBackground({ videoSrc, reverse }: { videoSrc: string; reverse: boolean }) {
  return (
    <div className="absolute inset-0 overflow-hidden">
      <video className="absolute inset-0 h-full w-full object-cover opacity-[0.18]" src={videoSrc} autoPlay loop muted playsInline />
      ...
      <motion.div
        ...
        className={`absolute ${reverse ? "-right-32 top-1/3 bg-fuchsia-500/20" : "-left-32 top-1/4 bg-cyan-400/20"} h-[500px] w-[500px] rounded-full blur-[120px]`}
      />
      <motion.div
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 1.5, delay: 0.3 }}
        className={`absolute ${reverse ? "-left-32 bottom-1/4 bg-cyan-400/20" : "-right-32 bottom-1/4 bg-fuchsia-500/20"} h-[600px] w-[600px] rounded-full blur-[120px]`}
      />
    </div>
  );
}
