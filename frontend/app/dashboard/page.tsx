...
const listingKeywords = Array.isArray(listingView?.keywords)
  ...
  const loadProfile = async (email: string) => {
    try {
      ...
      const res = await fetch(
         `http://127.0.0.1:8000/api/auth/me?email=${encodeURIComponent(email)}`
)
...
const ikpuPromise = searchIkpuAuto(ikpuSearchText)
    ...
    try {
      ...
      const res = await fetch(
        `http://127.0.0.1:8000/api/auth/activate-tariff?email=${encodeURIComponent(userEmail)}`,
        {
          method: "POST",
          headers: {
...
const ikpuPromise = searchIkpuAuto(ikpuSearchText)
  ...
  return
    ...
    const res = await fetch("http://127.0.0.1:8000/api/full-generate", {
      method: "POST",
      body: formData,
    })
  ...
  return
    ...
    const images = (data.slides || [])
      ...
      .map((s: any) => `http://127.0.0.1:8000${s.image_url}`)
  ...
  alert("Ошибка скачивания")
    ...
    const url = `http://127.0.0.1:8000/products/${createdProduct.id}/download-image?marketplace_mode=${selectedMarketplace}`
