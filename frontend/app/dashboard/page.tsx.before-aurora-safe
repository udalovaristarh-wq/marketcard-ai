...
const ikpuPromise = Promise.resolve()
  ...
  if (!emailToUse) {
    ...
    console.log("ACTIVATE 4 BEFORE FETCH")
      const res = await fetch(
        `https://marketcard.uz/api/payments/create-order?email=${encodeURIComponent(emailToUse)}&tariff_name=${encodeURIComponent(selectedTariff)}&provider=${encodeURIComponent(selectedPayment || "unknown")}`,
        {
          method: "POST",
        }
      )
...
>
  ...
  >
    <div
      style={{
        width: "22px",        border: "3px solid rgba(34,211,238,0.35)",
