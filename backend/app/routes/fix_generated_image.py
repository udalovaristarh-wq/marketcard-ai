...
""".strip()
    ...
    # 🔻 per-image fix counter (3 free fixes per original image)
    try:
        # Count fixes by root/original image name, not by every new _fixed_ file
        ...
        # only after 3 free fixes => charge 1 generation
        if fix_count > 3:
