import os
from pathlib import Path

from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware
from fastapi.staticfiles import StaticFiles

from app.db import create_db_and_tables
from app.routers import admin
from app.routers.admin_analytics import router as admin_analytics_router
from app.routers.admin_finance import router as admin_finance_router
from app.routers.analysis import router as analysis_router
from app.routers.auth import router as auth_router
from app.routers.click import router as click_router
from app.routers.payments import router as payments_router
from app.routers.products import router as products_router
from app.routers.system import router as system_router
from app.routes import (
    abc_analysis,
    card_audit,
    deficit_products,
    demo_generate,
    full_generate,
    support,
    translate_listing,
)
from app.routes.ai_generate import router as ai_router
from app.routes.fix_generated_image import router as fix_generated_image_router
from app.routes.ikpu import router as ikpu_router
from app.routes.listing_generate import router as listing_generate_router
from app.routes.product_intelligence import router as product_intelligence_router
from app.routes.product_photo_analyze import router as product_photo_analyze_router
from app.routes.queue_generate import router as queue_generate_router

BACKEND_DIR = Path(__file__).resolve().parent.parent
GENERATED_DIR = Path(
    os.getenv("GENERATED_CARDS_DIR", "/var/www/marketcard/generated_cards")
).resolve()
REPORTS_DIR = Path(
    os.getenv("GENERATED_REPORTS_DIR", "/var/www/marketcard/generated_reports")
).resolve()
TEMP_DIR = Path(os.getenv("TEMP_DIR", str(BACKEND_DIR / "temp"))).resolve()

GENERATED_DIR.mkdir(parents=True, exist_ok=True)
REPORTS_DIR.mkdir(parents=True, exist_ok=True)
TEMP_DIR.mkdir(parents=True, exist_ok=True)

_cors_origins = [
    "https://marketcard.uz",
    "https://www.marketcard.uz",
]
_extra_origins = os.getenv("CORS_EXTRA_ORIGINS", "")
if _extra_origins:
    _cors_origins.extend(
        origin.strip() for origin in _extra_origins.split(",") if origin.strip()
    )
if os.getenv("CORS_ALLOW_LOCALHOST", "1").lower() in ("1", "true", "yes"):
    _cors_origins.extend(
        [
            "http://localhost:3000",
            "http://127.0.0.1:3000",
            "http://localhost:3001",
            "http://127.0.0.1:3001",
        ]
    )

app = FastAPI(
    title="MarketCard AI",
    debug=os.getenv("DEBUG", "0").lower() in ("1", "true", "yes"),
    root_path="/api",
)

app.add_middleware(
    CORSMiddleware,
    allow_origins=_cors_origins,
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

app.mount("/generated_cards", StaticFiles(directory=str(GENERATED_DIR)), name="generated_cards")
app.mount("/generated_reports", StaticFiles(directory=str(REPORTS_DIR)), name="generated_reports")

app.include_router(auth_router, prefix="/auth", tags=["auth"])
app.include_router(payments_router)
app.include_router(click_router)
app.include_router(products_router)
app.include_router(ai_router)
app.include_router(listing_generate_router)
app.include_router(full_generate.router)
app.include_router(demo_generate.router)
app.include_router(product_photo_analyze_router)
app.include_router(queue_generate_router)
app.include_router(ikpu_router)
app.include_router(admin.router)
app.include_router(admin_finance_router)
app.include_router(admin_analytics_router)
app.include_router(support.router)
app.include_router(system_router)
app.include_router(fix_generated_image_router)
app.include_router(product_intelligence_router, prefix="/api")
app.include_router(analysis_router)
app.include_router(card_audit.router)
app.include_router(abc_analysis.router)
app.include_router(deficit_products.router)
app.include_router(translate_listing.router)


@app.on_event("startup")
def on_startup() -> None:
    create_db_and_tables()
